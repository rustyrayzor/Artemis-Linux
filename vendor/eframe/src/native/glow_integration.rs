...
impl GlutinWindowContext {
    ...
    unsafe fn new(
        ...
        */
        ...
        let requested_color_bits = native_options.color_buffer_bits.unwrap_or(8);
        let minimum_color_bits = requested_color_bits.min(8);
        let config_template_builder = config_template_builder
            .with_buffer_type(glutin::config::ColorBufferType::Rgb {
                r_size: minimum_color_bits,
                g_size: minimum_color_bits,
                b_size: minimum_color_bits,
            })
            // RGB10A2 configurations have two alpha bits. Request every alpha size while choosing
            // a deep-color config so the picker can retain 8-bit RGBA as an in-process fallback.
            .with_alpha_size(if requested_color_bits > 8 { 0 } else { 8 });
                ...
                .build(
                    ...
                    |config_iterator| {
                        ...
                        for config in config_iterator {
                            let requested_config = match config.color_buffer_type() {
                                Some(glutin::config::ColorBufferType::Rgb {
                                    r_size,
                                    g_size,
                                    b_size,
                                }) => {
                                    r_size >= requested_color_bits
                                        && g_size >= requested_color_bits
                                        && b_size >= requested_color_bits
                                }
                                _ => false,
                            };
                            if requested_color_bits <= 8 || requested_config {
                                log::debug!("using requested glutin color config: {config:?}");
                                return config;
                            }
                            fallback.get_or_insert(config);
                        }
                        let config = fallback.expect(
                            "failed to find a matching configuration for creating glutin config",
                        );
                        log::warn!(
                            "no {requested_color_bits}-bit color config is available; using the \
                             first compatible fallback: {config:?}"
                        );
                        config
                    },
                )
                .map_err(|error| {
                    crate::Error::NoGlutinConfigs(config_template_builder.build(), error)
                })?
        };
