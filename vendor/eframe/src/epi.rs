...
pub struct NativeOptions {
    ...
    /// Requests this many bits for each red, green, and blue channel when using the glow renderer.
    ///
    /// A value above eight permits two-bit alpha configurations so the config picker can select
    /// RGB10A2. If the requested configuration is unavailable, eframe uses its normal eight-bit
    /// RGBA configuration. The default is `None`, which preserves upstream behavior.
    #[cfg(feature = "glow")]
    pub color_buffer_bits: Option<u8>,
